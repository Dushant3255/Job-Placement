...
public class ProfilePictureScreen extends JFrame {
    ...
    private final String gender;     // only meaningful for students
    private final boolean isCompany; // ✅ company mode
    ...
    public ProfilePictureScreen(String email, String gender) {
        this(email, gender, false);
    }

    // Company constructor (convenient)
    public ProfilePictureScreen(String email) {
    ...
    private void initUI() {
        ...
        JPanel header = buildHeader(subtitle, e -> {
            // back to otp
            new VerifyOtpScreen(email, gender, isCompany).setVisible(true);
            dispose();
        });
        ...
        InteractiveButton browseBtn = new InteractiveButton("Browse Files", SECONDARY_BTN_BG);
        browseBtn.setForeground(SECONDARY_BTN_FG);
        browseBtn.addActionListener(e -> browseForImage());

        InteractiveButton removeBtn = new InteractiveButton("Remove", SECONDARY_BTN_BG);
        removeBtn.setForeground(SECONDARY_BTN_FG);
    ...
    private JPanel createDropArea() {
        ...
        String dropText = isCompany ? "Drop Logo Here" : "Drop Image Here";
        previewLabel = new JLabel(dropText, SwingConstants.CENTER);
    ...
    // gender key for student defaults
    ...
    private void applyDefaultAndFinish() {
        ...
        SwingWorker<BufferedImage, Void> worker = new SwingWorker<>() {
            ...
            protected BufferedImage doInBackground() throws Exception {
                ...
                // disk: resources/images/...
                if (img == null) {
                    for (String name : candidates) {
                        File diskFile = new File("resources/images/" + name);
                        if (diskFile.exists()) {
                ...
                if (img == null) throw new IllegalStateException("Default image not found");
                return img;
            }
    ...
    private void onSave() {
        // ✅ Company: if user didn't upload, auto-apply default logo
        ...
        String chosen = usingDefaultImage
                ? ("Default: " + defaultImageName)
                : ("File: " + selectedImageFile.getName());

        JOptionPane.showMessageDialog(
                this,
                (isCompany ? "Company logo saved!\n" : "Profile picture saved!\n") + chosen,
                "Success",
                JOptionPane.INFORMATION_MESSAGE
        );
